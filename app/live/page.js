...
import { getLiveData } from "@/app/_util/data";
...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        const user = {
            name: "Live Registrations Update",
            email: "admin@slts.cbe",
        };
        setUser(user);
        getLiveData().then((_data) => {
            if (_data.length === 8 && _data.every((x) => x.length === 0)) {
                setData([]);
                setFilteredData([]);
                return;
            }
                        ...
                        >
                            <div className="flex flex-col gap-2 px-4 py-2">
                                {row.entryMarked === true && (
                                    <p className="text-xs font-bold bg-[#c4ffc2] text-[#07210d] p-1 px-2 rounded-2xl w-full text-center">
                                        Entry Marked
                                    </p>
                                )}
