...
export default function DistrictStats() {
  ...
  useEffect(() => {
    ...
    getRegistrationData().then((_data) => {
      ...
      let groupedRows = Object.keys(groupedData).map((key) => {
      ...
      // get unique dates
      let _uniqueDates = groupedRows.map((group) => group.checkInDate);
    ...
    <div className="flex flex-col justify-center w-fit mx-auto">
      ...
      <div className="flex flex-wrap gap-2 my-4">
        {uniqueDates.map((date, idx) => (
          <button
            key={idx}
            className={`px-4 py-2 rounded-xl ${filterDate === date ? "bg-blue-500 text-white" : "bg-gray-200"
              }`}
                    ...
                    <div className="pt-2">
                      ...
                      <p className="text-4xl font-bold">
                        {r.rows.reduce(
                          (acc, row) =>
                            acc +
                            ((row.needsAccommodation === "Yes" || parseInt(row.numMaleAccompanyingNeedAccommodation) > 0) &&
                              row.gender === "Male"
                              ? 1
                              : 0) +
                            (parseInt(
                    ...
                    <div className="pt-2">
                      ...
                      <p className="text-4xl font-bold">
                        {r.rows.reduce(
                          (acc, row) =>
                            acc +
                            ((row.needsAccommodation === "Yes" || parseInt(row.numFemaleAccompanyingNeedAccommodation) > 0) && row.gender === "Female"
                              ? 1
                              : 0) +
                            (parseInt(
                    ...
                    <div className="pt-2 bg-gray-100 p-2 px-8 rounded-2xl">
                      ...
                      <p className="text-4xl font-bold">
                        {r.rows.reduce(
                          (acc, row) =>
                            acc +
                            ((row.needsAccommodation === "Yes" || parseInt(row.numMaleAccompanyingNeedAccommodation) > 0 || parseInt(row.numFemaleAccompanyingNeedAccommodation) > 0)  ? 1 : 0) +
                            (parseInt(
                              row.numMaleAccompanyingNeedAccommodation
                            ) || 0) +
                            (parseInt(
